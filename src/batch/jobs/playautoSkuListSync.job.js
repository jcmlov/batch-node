...
exports.run = async (type = "SKU_LIST_SYNC") => {
  ...
  const stat = { totalCnt: 0, successCnt: 0, failCnt: 0 };
  ...
  function isOptionsChanged(dbOptions, apiOptionsNorm) {
    if ((dbOptions?.length || 0) !== (apiOptionsNorm?.length || 0)) return true;
  ...
  async function replaceOptions(skuId, apiOptionsNorm, isNewSku) {
    ...
    for (const opt of apiOptionsNorm) {
      await client.query(
        ...
        `,
        [skuId, opt.optnSeq, opt.optnNm, opt.optnTypeCd, opt.optnValCn, ACTOR],
      );
    }

    // 신규면 INIT 스냅샷도 남김(요구: HIS에도 데이터 입력)
  ...
  try {
    ...
    while (true) {
      ...
      for (const item of results) {
        ...
        try {
          ...
          // 배송처 재고 동기화 + his
          await replaceDepotStocks(skuId, item.sol_no, item.depots, isNewSku);
    ...
    console.error("❌ [JOB][PLAYAUTO][SKU_LIST] 실패:", err);
    throw err;
  } finally {
    try {
